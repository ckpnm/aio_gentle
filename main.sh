...
step_base_deps() {
    echo -e "\n${C_ACCENT}=== Установка базовых утилит ===${C_BASE}"
    _do_base_deps() {
        apt-get update -y
...
step_bbr_ipv6() {
    echo -e "\n${C_ACCENT}=== Оптимизация сети (BBR и IPv6) ===${C_BASE}"
    _do_bbr_ipv6() {
...
step_ssh_port() {
    echo -e "\n${C_ACCENT}=== Настройка SSH порта ===${C_BASE}"
...
step_ufw_setup() {
    echo -e "\n${C_ACCENT}=== Брандмауэр UFW ===${C_BASE}"
    read -p "Если вы меняли порт SSH, укажите его для открытия в UFW (по умолчанию 22): " UFW_SSH
    UFW_SSH=${UFW_SSH:-22}
    _do_ufw() {
...
step_fail2ban_setup() {
    echo -e "\n${C_ACCENT}=== Установка Fail2Ban ===${C_BASE}"
    _do_fail2ban() {
...
step_caddy_selfsteal() {
    echo -e "\n${C_ACCENT}=== Caddy Selfsteal ===${C_BASE}"
...
step_remnanode_setup() {
    echo -e "\n${C_ACCENT}=== Развертывание Remnanode ===${C_BASE}"
...
step_logrotate_xray() {
    echo -e "\n${C_ACCENT}=== Ротация логов Xray ===${C_BASE}"
    _do_logrotate() {
...
step_traffic_guard_setup() {
    echo -e "\n${C_ACCENT}=== Инсталляция Traffic Guard ===${C_BASE}"
    _do_tg() {
...
step_warp_setup() {
    echo -e "\n${C_ACCENT}=== Cloudflare WARP ===${C_BASE}"
    _do_warp() {
...
step_show_reality() {
    echo -e "\n${C_ACCENT}=== Ключи Reality (x25519) и Статус ===${C_BASE}"
