...
EOF
    ...
    echo -e "  ${C_OK}[ ИНФО ]${C_BASE} Забанено $(ipset list leaseweb_v4 2>/dev/null | grep -c '/') IPv4 и $(ipset list leaseweb_v6 2>/dev/null | grep -c '/') IPv6 подсетей."
}
...
step_ipregion() {
    ...
    # Записываем скрипт во временный файл без интерпретации переменных внутри heredoc
...
lookup_gemini_supported() {
  ...
  if grep_wrapper -qi "^- ${country_name}$" <<<"$regions_md"; then
    available="Yes"; color_name="SERVICE"
  else
    available="No"; color_name="HEART"
  fi
...
lookup_reddit_guest_access() {
  ...
  if [[ "$response" != "Denied" ]]; then
    is_available="Yes"; color_name="SERVICE"
  else
    is_available="No"; color_name="HEART"
  fi
...
lookup_google_search_captcha() {
  ...
  if [[ -z "$is_captcha" ]]; then
    is_captcha="No"; color_name="SERVICE"
  else
    is_captcha="Yes"; color_name="HEART"
  fi
